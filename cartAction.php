...
");
        ...
        $itemData = array(
            'id' => $row['id'],
            'name' => $row['name'],
            'price' => $row['prijsDagProduct'],
            'qty' => 1
        
        );
        $query = $db->query("UPDATE products SET procentKortingDagProduct = procentKortingDagProduct-0.01");
    ...
    }elseif($_REQUEST['action'] == 'favoriet' && !empty($_REQUEST['id'])){
        $productID = $_REQUEST['id'];
        // get product details
        $query = $db->query("SELECT * FROM products WHERE id = ".$productID);
        $row = $query->fetch_assoc();
        ...
        $insertItem = $cart->insert($itemData);
        $redirectLoc = $insertItem?'favorieten.php':'index.php';
        header("Location: ".$redirectLoc);
